...
import {
  CashbookTransferItem,
  cashbookService,
  CashbookTransfersListResponse,
} from "@/api/cashbooks.service";
import { getColumns } from "@/components/cashbook-transfers/columns";

export default function CashbookTransfersListPage() {
  const router = useRouter();
  const [transfers, setTransfers] = useState<CashbookTransferItem[]>([]);
  ...
  const { request, loading, error } = useApi<CashbookTransfersListResponse>();

  const loadTransfers = useCallback(async () => {
    const res = await request(() =>
      cashbookService.getTransfers({
  ...
  }, [search, status, page, loadTransfers]);
  ...
  const handleAdd = () => {
    router.push("/auth/cashbook-transfers/add");
  };

  const handleView = (transfer: CashbookTransferItem) => {
    router.push(`/auth/cashbook-transfers/${transfer.id}`);
  };

  const handleEdit = (transfer: CashbookTransferItem) => {
    router.push(`/auth/cashbook-transfers/${transfer.id}/edit`);
  };
  ...
  return (
    <div className="space-y-6 relative min-h-[400px]">
      <BaseFilter
        ...
        placeholder="Search transfer references..."
