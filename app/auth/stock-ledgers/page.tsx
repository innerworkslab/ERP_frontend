...
import { StockLedger, stockLedgerService } from "@/api/stockLedgers.service";
import { getColumns } from "@/components/stock-ledgers/columns";
...
export default function StockLedgerPage() {
  ...
  const { request, loading, error } = useApi<ApiResponse<StockLedger[]>>();

  const loadLedgers = useCallback(async () => {
    const res = await request(() =>
      stockLedgerService.getAll({
  ...
  useEffect(() => {
    const timer = setTimeout(loadLedgers, 400);
    return () => clearTimeout(timer);
  }, [search, page, loadLedgers]);

  const handleView = useCallback(
    (ledger: StockLedger) => {
      const type =
        ledger.reference_type === "opening_stock"
          ? "opening-stocks"
          : "stock-transfers";
      router.push(`/inventory/${type}/${ledger.reference_id || ""}`);
    },
    [router],
  );
  ...
  return (
    <div className="space-y-6 relative min-h-[400px]">
      <BaseFilter
        ...
        placeholder="Search SKU, Product or Voucher..."
      />
